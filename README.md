...
**Il mio Museo Neroverde** permette di trasformare partite, giocatori e stagioni in memorie personali con emozione, intensità, momento preferito e nota. Il tour fullscreen crea una linea del tempo, sale tematiche e una costellazione privata. Tutto resta nel `localStorage` del browser: non esiste profilo pubblico, non vengono chiamate API e il contenuto non entra nella telemetria. La pagina Museo offre backup JSON, import con deduplica e cancellazione esplicita; modello e limiti sono descritti in [`docs/product/PERSONAL_MUSEUM.md`](docs/product/PERSONAL_MUSEUM.md).
