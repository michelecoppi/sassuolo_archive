...
execFileSync(process.execPath,[path.resolve('node_modules/tsx/dist/cli.mjs'),path.resolve('scripts/init-db.ts')],{cwd:process.cwd(),env:childEnvironment,stdio:'pipe'});
...
test('la fixture schema 8 migra alla versione corrente conservando gli invarianti',()=>{
  assert.equal(getSchemaVersion(),10);assert.equal(String(db.pragma('integrity_check',{simple:true})),'ok');assert.equal(String(db.pragma('foreign_key_check',{simple:true})??''),'');
  ...
  initDb();assert.equal((db.prepare(`SELECT COUNT(*) AS total FROM schema_migrations WHERE version=10`).get() as any).total,1);
});
...
test('i dati della versione precedente possono essere ripristinati nel nuovo schema',()=>{
  ...
  assert.equal((db.prepare(`SELECT COUNT(*) AS total FROM players WHERE name='Giocatore fixture schema 8'`).get() as any).total,1);assert.equal(getSchemaVersion(),10);assert.equal(String(db.pragma('integrity_check',{simple:true})),'ok');
});
