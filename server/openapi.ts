...
const baseOpenApiDocument={
  ...
  paths:{
    ...
    '/status':{get:{tags:['Archive'],operationId:'getStatus',responses:{'200':{description:'Stato pubblico, identità del dataset e changelog verificabile',content:{'application/json':{schema:{type:'object',required:['status','dataset','summary','entries'],properties:{status:{type:'object'},dataset:{type:'object'},summary:{type:'object'},entries:{type:'array',items:{type:'object'}}}}}}},'503':errorResponse}}},
    '/status/feed.xml':{get:{tags:['Archive'],operationId:'getStatusFeedXml',responses:{'200':{description:'Feed RSS 2.0 del changelog pubblico',content:{'application/rss+xml':{schema:{type:'string'}}}},'503':errorResponse}}},
    ...
    '/time-machine':{get:{tags:['Archive'],operationId:'getTimeMachine',responses:{'200':{description:'Viaggio stagionale data-driven dal 2007/08: riepilogo, protagonisti, migliore vittoria, percorso punti, copertura e fonti. I dati assenti restano null.'}}}},
