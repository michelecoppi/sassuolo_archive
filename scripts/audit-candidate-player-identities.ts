...
const normalized=db.prepare(`SELECT id,name FROM players WHERE lower(replace(replace(replace(trim(name),'.',''),' ',''),'-',''))=?`);
...
const report=rows.map((row,index)=>{
  ...
  const similar=normalized.all(normalize(name)) as {id:number;name:string}[];
