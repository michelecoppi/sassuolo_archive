...
export function ExperienceProvider({children}:{children:ReactNode}){
  ...
  useEffect(()=>{const update=()=>setP(x=>({...x,offline:!navigator.onLine}));const cacheUpdate=()=>setCacheRevision(value=>value+1);window.addEventListener('online',update);window.addEventListener('offline',update);window.addEventListener('sassuolo-cache-updated',cacheUpdate);return()=>{window.removeEventListener('online',update);window.removeEventListener('offline',update);window.removeEventListener('sassuolo-cache-updated',cacheUpdate)};},[]);
