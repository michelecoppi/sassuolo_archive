...
type Row=Player&{season_appearances:number|null;season_starts:number|null;season_minutes:number|null;season_goals:number|null;season_assists:number|null;season_rating:number|null;season_rating_source:'sassuolo-archive'|'provider'|null;archive_rating_matches:number|null;archive_rating_confidence:number|null};
...
export default function CurrentSquad(){
  ...
  return <><PageTitle eyebrow={d.season?`Stagione corrente · ${d.season}`:'Stagione corrente'} title="Rosa attuale" subtitle={`Rosa e statistiche della stagione configurata in CURRENT_SEASON · ${competitionLabel}.`} action={<div className="flex flex-wrap gap-2"><Link className="btn-secondary" to="/current-season"><CalendarClock className="h-4 w-4"/>Partite e calendario</Link><button className="btn-primary" disabled={busy} onClick={update}><RefreshCw className={`h-4 w-4 ${busy?'animate-spin':''}`}/>Sincronizza provider (opzionale)</button></div>}/>
    {!d.players.length?<Empty text="Nessuna rosa corrente memorizzata. Inserisci i giocatori dal Data Manager oppure usa una sincronizzazione disponibile."/>:<div className="space-y-6">{groups.map(([position,rows])=><section key={position}><h2 className="mb-3 text-lg font-bold">{position}</h2><div className="table-wrap"><table><thead><tr><th>#</th><th>Giocatore</th><th>Nazionalità</th><th>Pres.</th><th>Tit.</th><th>Min.</th><th>Gol</th><th>Assist</th><th>Voto</th></tr></thead><tbody>{rows.map(p=><tr key={p.id}><td>{fmt(p.shirt_number)}</td><td><Link className="flex items-center gap-3 font-bold text-neroverde-400 hover:underline" to={`/players/${p.id}`}><RemoteImage src={p.photo_url} className="h-10 w-10 rounded-full bg-zinc-800 object-cover" alt={`Foto di ${p.name}`} width={40} height={40}/>{p.name}</Link></td><td>{fmt(p.nationality)}</td><td>{fmt(p.season_appearances)}</td><td>{fmt(p.season_starts)}</td><td>{fmt(p.season_minutes)}</td><td>{fmt(p.season_goals)}</td><td>{fmt(p.season_assists)}</td><td>{p.season_rating==null?'N/D':<><b className={p.season_rating_source==='sassuolo-archive'?'text-neroverde-300':''}>{Number(p.season_rating).toFixed(2)}</b><div className="text-[10px] text-zinc-400">{p.season_rating_source==='sassuolo-archive'?`Archivio · ${p.archive_rating_matches} gare · ${Math.round(Number(p.archive_rating_confidence??0)*100)}%`:'Fonte esterna'}</div></>}</td></tr>)}</tbody></table></div></section>)}</div>}
  </>;
}
