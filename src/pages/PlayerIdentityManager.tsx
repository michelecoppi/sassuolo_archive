  ...
  return <>
    ...
    {!conflicts.length?<Empty title="Nessun candidato da gestire"/>:<div className="space-y-4">{conflicts.map(c=>{
      ...
      return <article className="card p-5" key={c.id}>
        <div className="flex items-start justify-between gap-3"><div><div className="text-xl font-black">{c.raw_name}</div><div className="mt-1 text-sm text-zinc-500">{c.reason??'Possibile duplicato'} · {c.source_provider??'Fonte non indicata'}{c.source_player_id?` · ID ${c.source_player_id}`:''}</div><div className="mt-1 text-xs text-zinc-600">Contesto: {c.context??'non indicato'}</div></div><button className="btn-secondary !border-red-500/30 !text-red-300" disabled={busy===c.id} onClick={()=>act(c,'reject')}>Rifiuta</button></div>
        ...
        <div className="mt-3 flex items-center justify-between rounded-xl border border-zinc-800 p-3"><div><b>Persona realmente nuova?</b><p className="text-xs text-zinc-500">Crea un profilo vuoto con alias e ID fonte; nessuna statistica viene importata qui.</p></div><button className="btn-secondary" disabled={busy===c.id} onClick={()=>act(c,'create')}>Crea nuovo profilo</button></div>
      </article>
    })}</div>}
  </>;
}
