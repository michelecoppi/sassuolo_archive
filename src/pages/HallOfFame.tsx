...
export default function HallOfFame() {
  ...
  return <><PageTitle title="Hall of Fame" subtitle="Classifiche calcolate dal database locale. N/D significa dato non disponibile, non zero." />
    ...
    <div className="grid gap-4 lg:grid-cols-2"><Ranking title="Più presenze" rows={data.appearances} field="appearances" /><Ranking title="Più gol" rows={data.goals} field="goals" /><Ranking title="Più assist" rows={data.assists} field="assists" /><Ranking title="Più minuti" rows={data.minutes} field="minutes" /><Ranking title="Più clean sheet" rows={data.clean_sheets} field="clean_sheets" /></div>
    <section className="mt-6"><h2 className="mb-3 text-xl font-black text-amber-200">Statistiche negative</h2><p className="mb-4 text-sm text-zinc-500">Leader per autogol, cartellini gialli, cartellini rossi e falli commessi.</p><div className="grid gap-4 lg:grid-cols-2"><Ranking title="Più autogol" rows={negative?.own_goals ?? data.own_goals} field="own_goals" /><Ranking title="Più cartellini gialli" rows={negative?.yellow_cards ?? []} field="yellow_cards" /><Ranking title="Più cartellini rossi" rows={negative?.red_cards ?? []} field="red_cards" /><Ranking title="Più falli commessi" rows={negative?.fouls_committed ?? []} field="fouls_committed" /></div></section>
    {!!data.teamOwnGoals?.length && <section className="mt-6 card p-5"><h2 className="mb-3 font-bold">Autogol a favore / contro</h2><div className="table-wrap"><table><thead><tr><th>Stagione</th><th>Competizione</th><th>A favore</th><th>Contro</th></tr></thead><tbody>{data.teamOwnGoals.map(row => <tr key={`${row.season}-${row.competition}`}><td>{row.season}</td><td>{row.competition}</td><td>{fmt(row.own_goals_for)}</td><td>{fmt(row.own_goals_against)}</td></tr>)}</tbody></table></div></section>}
  </>;
}
