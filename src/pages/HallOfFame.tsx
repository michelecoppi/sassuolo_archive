...
export default function HallOfFame() {
  ...
  return <><PageTitle title="Hall of Fame" subtitle="Classifiche calcolate dal database locale. N/D significa dato non disponibile, non zero." />
    ...
    <section className="mt-6"><h2 className="mb-3 text-xl font-black text-amber-200">Statistiche negative</h2><p className="mb-4 text-sm text-zinc-400">Leader per autogol, cartellini gialli, cartellini rossi e falli commessi.</p><div className="grid gap-4 lg:grid-cols-2"><Ranking title="Più autogol" rows={negative?.own_goals ?? data.own_goals} field="own_goals" definition={definition.own_goals}/><Ranking title="Più cartellini gialli" rows={negative?.yellow_cards ?? []} field="yellow_cards" definition={definition.yellow_cards}/><Ranking title="Più cartellini rossi" rows={negative?.red_cards ?? []} field="red_cards" definition={definition.red_cards}/><Ranking title="Più falli commessi" rows={negative?.fouls_committed ?? []} field="fouls_committed" definition={definition.fouls_committed}/></div></section>
