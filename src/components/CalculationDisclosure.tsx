...
export function CalculationContext({lastRecalculation,perimeter,items}:{lastRecalculation:string|null;perimeter:string;items:{label:string;value:string|number}[]}){
  return <aside className="card mb-5 p-4" aria-label="Copertura del calcolo"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h2 className="font-bold">Copertura del calcolo</h2><p className="mt-1 text-sm text-zinc-400">{perimeter}</p></div><div className="text-xs text-zinc-500">Ultimo ricalcolo: <b className="text-zinc-300">{lastRecalculation?new Date(lastRecalculation).toLocaleString('it-IT'):'N/D'}</b></div></div><dl className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{items.map(item=><div className="rounded-xl bg-zinc-950/60 p-3" key={item.label}><dt className="text-xs text-zinc-500">{item.label}</dt><dd className="mt-1 font-bold text-white">{item.value}</dd></div>)}</dl><Link className="mt-3 inline-flex text-xs font-bold text-neroverde-300 hover:underline" to="/methodology#formulas">Metodologia generale →</Link></aside>;
}
