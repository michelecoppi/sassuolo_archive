...
export function Breadcrumb({items}:{items:{label:string;to?:string}[]}){return <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-2 text-xs font-medium text-zinc-400">{items.map((item,index)=><span className="flex items-center gap-2" key={`${item.label}-${index}`}>{index>0&&<span aria-hidden="true">/</span>}{item.to?<Link className="hover:text-neroverde-300" to={item.to}>{item.label}</Link>:<span className="text-zinc-300">{item.label}</span>}</span>)}</nav>}
...
export function StatCard({label,value,hint,competition,title}:{label:string;value:ReactNode;hint?:string;competition?:string|null;title?:string}){return <div title={title} className={`card p-4 ${competition?competitionAccent(competition).card:''}`}><div className="flex items-start justify-between gap-2"><div className="text-[11px] font-bold uppercase tracking-wider text-zinc-400">{label}</div>{competition&&<CompetitionBadge competition={competition} compact/>}</div><div className="mt-3 text-2xl font-black tracking-tight text-white md:text-3xl">{value ?? 'N/D'}</div>{hint&&<div className="mt-1.5 text-xs text-zinc-400">{hint}</div>}</div>}
export function Empty({title='Nessun dato disponibile',text='Importa i dati storici o usa Data Manager per aggiornare le fonti disponibili.',action}:{title?:string;text?:string;action?:ReactNode}){return <div className="card flex min-h-48 flex-col items-center justify-center p-8 text-center"><div className="mb-4 grid h-12 w-12 place-items-center rounded-2xl border border-neroverde-400/20 bg-neroverde-400/10 text-neroverde-300"><Inbox className="h-5 w-5"/></div><h3 className="font-bold text-white">{title}</h3><p className="mt-2 max-w-lg text-sm leading-6 text-zinc-400">{text}</p>{action&&<div className="mt-4">{action}</div>}</div>}
...
export function ErrorState({message='Non è stato possibile caricare questi dati.',retry}:{message?:string;retry?:()=>void}){return <div className="card flex min-h-48 flex-col items-center justify-center p-8 text-center"><h3 className="font-bold text-white">Qualcosa non ha funzionato</h3><p className="mt-2 text-sm text-zinc-400">{message}</p>{retry&&<button className="btn-secondary mt-4" onClick={retry}>Riprova</button>}</div>}
...
export function Pagination({page,pageSize,total,onPage}:{page:number;pageSize:number;total:number;onPage:(page:number)=>void}){
  const pages=Math.max(1,Math.ceil(total/pageSize));if(total<=pageSize)return <p className="mt-4 text-sm text-zinc-400">{total} risultati</p>;
  return <nav aria-label="Paginazione" className="mt-4 flex items-center justify-between gap-3 text-sm"><span className="text-zinc-400">{total} risultati · pagina {page} di {pages}</span><div className="flex gap-2"><button className="btn-secondary" disabled={page<=1} onClick={()=>onPage(page-1)}>Precedente</button><button className="btn-secondary" disabled={page>=pages} onClick={()=>onPage(page+1)}>Successiva</button></div></nav>;
}
type RemoteImageProps=Omit<ImgHTMLAttributes<HTMLImageElement>,'src'>&{src?:string|null;fallback?:ReactNode};
export function RemoteImage({src,alt='',fallback,className='',width=96,height=96,...props}:RemoteImageProps){
  const[failed,setFailed]=useState(false);const remote=Boolean(src&&/^https?:\/\//i.test(src));const resolved=remote?`/api/assets/image?url=${encodeURIComponent(src!)}`:src;
  if(!resolved||failed)return <>{fallback??<span className={`grid place-items-center bg-zinc-800 text-zinc-400 ${className}`} role="img" aria-label={alt||'Immagine non disponibile'}><UserRound aria-hidden="true" className="h-1/2 w-1/2"/></span>}</>;
  return <img {...props} src={resolved} alt={alt} width={width} height={height} loading="lazy" decoding="async" className={className} onError={()=>setFailed(true)}/>;
}
...
export function SeasonHeader({season,competition,children}:{season:string;competition?:string|null;children?:ReactNode}){return <section className={`card mb-5 overflow-hidden p-5 md:p-6 ${competitionAccent(competition).card}`}><div className="mb-3"><CompetitionBadge competition={competition}/></div><div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="text-xs font-bold uppercase tracking-[.16em] text-zinc-400">Archivio stagione</div><h2 className="mt-1 text-3xl font-black text-white">{season}</h2></div>{children}</div></section>}
export function PlayerCard({name,photo,meta,children}:{name:string;photo?:string|null;meta?:ReactNode;children?:ReactNode}){return <article className="card flex items-center gap-3 p-4"><div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-zinc-800 font-black text-neroverde-300"><RemoteImage className="h-full w-full object-cover" src={photo} alt={`Foto di ${name}`} width={48} height={48}/></div><div className="min-w-0"><div className="truncate font-bold text-white">{name}</div>{meta&&<div className="mt-1 text-xs text-zinc-400">{meta}</div>}{children}</div></article>}
